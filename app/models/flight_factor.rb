class FlightFactor < ActiveRecord::Base
  ...
  def self.find_by_distance(distance)
    ...
    where("((lower_limit IS NULL) AND (upper_limit > ?)) OR ((lower_limit <= ?) AND (upper_limit > ?)) OR ((lower_limit <= ?) AND (upper_limit IS NULL))", distance, distance, distance, distance).first
  end
