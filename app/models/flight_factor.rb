class FlightFactor < ActiveRecord::Base
  ...
  def self.find_by_distance(distance)
    ...
    find :first, :conditions => [ "((lower_limit IS NULL) AND (upper_limit > ?)) OR ((lower_limit <= ?) AND (upper_limit > ?)) OR ((lower_limit <= ?) AND (upper_limit IS NULL))", distance, distance, distance, distance]
  end
